...
<!-- Insert Design Modal -->
              ...
              <form method="POST" id="ins_rec">
                <div class="modal-body">
                  <div class="form-group">
                    ...
                    <select type="select" name="id_kab" class="form-control" placeholder="Nama Kabupaten">
                      <option value="" selected>-Pilih Kabupaten-</option>
                      <?php
                      $select = $admin_fun->select("kabupaten");
                      if($select){ foreach($select as $se_data){
                      ?>
                      <option value="<?=$se_data['id_kab'];?>"><?=$se_data['id_kab']." | ".$se_data['nm_kab'];?></option>
                      <?php }}else{ } ?>
                    </select>
                  ...
                  <div class="form-group">
                  	...
                  	<input type="number" name="id_kec" class="form-control" placeholder="ID Kecamatan">
                  	<span class="error-msg" id="msg_2"></span>
                  </div>
                  <div class="form-group">
                  	<label><b>Nama Kecamatan</b></label>
                  	<input type="text" name="name_kec" class="form-control" placeholder="Nama Kecamatan">
          ...
          <div class="modal-content">
            ...
            <form method="POST" id="updata">
              <div class="modal-body">
                <div class="form-group">
                  <label><b>Nama Kabupaten</b></label>
                  <select type="select" name="u_id_kab" class="form-control" id="upd_1" placeholder="Nama Kabupaten">
                    <?php
                    $select = $admin_fun->select("kabupaten");
                    if($select){ foreach($select as $se_data){
                    ?>
                ...
                <div class="form-group">
                  <label><b>Nama Kecamatan</b></label>
                  <input type="text" class="form-control" name="u_name_kec" id="upd_2" placeholder="Nama Kecamatan">
    ...
    $('#tbl_rec').load('<?php echo $admin_url ?>model/daerah/kecamatan/v.php');
    ...
    $('#ins_rec').on("submit", function(e){
      ...
      $.ajax({
        ...
        type:'POST',
        url:'<?php echo $admin_url ?>controller/daerah/kec_control.php',
        data:$(this).serialize(),
    ...
    $(document).on("click", "button.editdata", function(){
      ...
      $.getJSON("<?php echo $admin_url ?>controller/daerah/kec_control.php", {checkid : check_id}, function(json){
       if(json.status == 0){
        $('#upd_1').val(json.idKab);
        $('#upd_2').val(json.nmKec);
        $('#upd_3').val(check_id);
      }
      else{
        console.log(json.msg);
